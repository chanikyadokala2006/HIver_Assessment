...
class LLMGroundednessJudgeResult(BaseModel):
    score: int = Field(description="Score from 1 to 5 rating the overall performance of the AI Agent.")
    reasoning: str = Field(description="Detailed reasoning for the score covering intent classification, escalation decision, and draft reply groundedness.")

def create_groundedness_judge():
    llm = ChatGoogleGenerativeAI(
        model="gemini-3.6-flash"
    ).with_structured_output(LLMGroundednessJudgeResult)
    return llm

def main():
    parser = argparse.ArgumentParser(description="Live AI Support Agent Evaluation")
    parser.add_argument("--brand", type=str, default="AppleSupport", help="Brand name to evaluate (default: AppleSupport)")
    parser.add_argument("--input", type=str, default="test_queries.txt", help="Text file containing one query per line")
    ...
    print("="*80)
    print(f" LIVE SUPPORT AGENT EVALUATION - Brand: {BRAND_NAME}")
    print("="*80)
    ...
    judge_llm = create_groundedness_judge()
    
    results = []
    
    print(f"\nEvaluating {len(queries)} custom queries...\n")
    
    for idx, user_msg in enumerate(queries):
        print("-" * 80)
        print(f"--- Query {idx+1}/{len(queries)} ---")
        print(f"User Issue: {user_msg}\n")
        
        # 1. Generate Prediction
        print("  [1/2] Invoking Support Agent (RAG + LLM)...")
        try:
        ...
        # 2. Call LLM Groundedness Judge
        print("  [2/2] Invoking LLM Groundedness Judge...")
        judge_prompt = f"""
        You are an incredibly strict, brutal, and unforgiving QA evaluator for customer support at {BRAND_NAME}.
        Your job is to scrutinize the AI Agent's performance on a completely new user query. Since there is no absolute 'Ground Truth' for this new query, you must evaluate the Agent's Draft Reply against the RETRIEVED CONTEXT (the historical data the Agent used).
        
        INPUT DATA:
        - Brand: {BRAND_NAME}
        - User Issue: {user_msg}
        
        RETRIEVED CONTEXT FROM VECTOR DB (Historical precedents):
        {retrieved_context}
        ...
        2. ESCALATION DECISION (MOST IMPORTANT): Did the AI make the right routing call? 
           - If the RETRIEVED CONTEXT does NOT provide a safe, confident resolution for the issue, the Agent MUST Escalate. 
           - If the context DOES provide a clear template/solution, it should Auto-handle.
        3. DRAFT REPLY: 
        ...
        # 3. Human Evaluation
        human_score_input = input("Please rate the AI Agent's performance on a scale of 1-5 (or 'q' to quit): ")
    ...
    if results:
        ...
        out_df.to_csv("live_evaluation_results.csv", index=False)
        print("\n" + "="*80)
        print(" LIVE EVALUATION COMPLETE")
        print("="*80)
        print("Saved to live_evaluation_results.csv")
